...
@section('header2','Ajouter Employee')

@section('subcontent')
<div class="flex justify-center p-6 bg-white border rounded-md ">


    <form action="{{ route('employees.store') }}" class="w-[1000px]" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="prenom" id="prenom" class="w-full px-4 py-2 mt-1 border border-gray-500 rounded-md focus:ring focus:ring-blue-300">
            @error('prenom')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" name="email" id="email" class="w-full px-4 py-2 mt-1 border border-gray-500 rounded-md focus:ring focus:ring-blue-300">
            @error('email')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="date_travail" id="date_travail" class="w-full px-4 py-2 mt-1 border border-gray-500 rounded-md focus:ring focus:ring-blue-300">
            @error('date_travail')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="flex ">
                <select name="poste" id="poste" class="w-full px-4 py-2 mt-1 border border-gray-500 rounded-md focus:ring focus:ring-blue-300">
                    <option value="">-- Choisir un team --</option>
                    @foreach ($teams as $teamOption)
                    <option value="{{ $teamOption->nom }}">
                        {{ $teamOption->nom }}
                    </option>
                    @endforeach
                </select>
                <a href="{{ route('team.create') }}" class="flex items-center justify-center w-12 h-12 ml-2 bg-blue-500 rounded-md">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="salaire" id="salaire" class="w-full px-4 py-2 mt-1 border border-gray-500 rounded-md focus:ring focus:ring-blue-300">
            @error('salaire')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
